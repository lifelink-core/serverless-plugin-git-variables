...
let _gitIndexLockClear = (() => {
  var _ref2 = (0, _asyncToGenerator3.default)(function* () {
    return new _promise2.default((() => {
      var _ref3 = (0, _asyncToGenerator3.default)(function* (resolve) {
        const gitDir = yield _exec('git rev-parse --git-dir');
        const gitIndexLock = `${gitDir.replace(/\s*/, '')}/index.lock`;
        _fs2.default.access(gitIndexLock, function (err) {
          if (err) {
            // No lock found
            resolve();
            return;
          ...
          const watcher = _fs2.default.watch(gitIndexLock);
          watcher.on('change', function (eventType) {
            if (eventType === 'rename') {
              try {
                _fs2.default.accessSync(gitIndexLock);
              } catch (e) {
                // File does not exist
                watcher.close();
                resolve();
  ...
  return function _gitIndexLockClear() {
